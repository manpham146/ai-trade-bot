  ...
  private async makePredictionRequest(marketData: MarketData): Promise<GeminiPrediction> {
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        Logger.warn(`Gemini API attempt ${attempt}/${this.config.maxRetries} failed:`, error as any);
...
`;
  ...
  private extractSignalFromText(text: string, marketData: MarketData): GeminiPrediction {
    ...
    if (upperText.includes('BUY') && !upperText.includes('NOT BUY') && !upperText.includes('DON\'T BUY')) {
      signal = 'BUY';
      confidence = 65;
      reasoning = 'Text analysis suggests bullish sentiment';
    } else if (upperText.includes('SELL') && !upperText.includes('NOT SELL') && !upperText.includes('DON\'T SELL')) {
      signal = 'SELL';
      confidence = 65;
    ...
    if (marketData.rsi > 70) confidence = Math.max(confidence - 10, 30);
    if (marketData.rsi < 30) confidence = Math.max(confidence - 10, 30);
  ...
  private analyzeTrend(data: MarketData): string {
    if (data.price > data.sma20 && data.sma20 > data.sma50) return 'Uptrend';
    if (data.price < data.sma20 && data.sma20 < data.sma50) return 'Downtrend';
    return 'Sideways';
  }
  ...
  private validateSignal(signal: any): 'BUY' | 'SELL' | 'HOLD' {
    if (['BUY', 'SELL', 'HOLD'].includes(signal)) return signal;
    return 'HOLD';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.callGeminiAPI('Hello, this is a connection test. Please respond with "Connection successful".');
